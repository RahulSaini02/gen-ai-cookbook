...
def embed_batch(texts: list[str], max_retries: int = 5) -> np.ndarray:
    """Embed a batch of texts and return a (len(texts), 1536) array.

    Retries with exponential backoff on 429s, since the embeddings endpoint
    rate-limits by tokens/min rather than requests/min, so a fixed sleep
    between batches isn't enough to avoid them.
    """
...
def build_index(
    doc_texts: list[str], batch_size: int = 256, sleep_seconds: float = 0.5
) -> np.ndarray:
    ...
    for i in tqdm(range(0, len(doc_texts), batch_size), desc="Embedding"):
        ...
        time.sleep(sleep_seconds)  # throttle to stay under the rate limit
