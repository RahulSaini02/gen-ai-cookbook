...
def parse_html_content_fine_grained(html_content):
    ...
    # Get article title
    article_title = (
        soup.find("title").get_text().strip() if soup.find("title") else "Untitled"
    )
...
def similarity_search(
    query_embed, target_embeddings, content_list, k=5, threshold=0.05, temperature=0.5
):
    """
...
def construct_prompt(query, text_results, image_results):
    ...
    for text in text_results:
        if text_results:
            text_context = (
                text_context + "**Article title:** " + text["article_title"] + "\n"
            )
    ...
    for image in image_results:
        if image_results:
            image_context = (
                image_context + "**Article title:** " + image["article_title"] + "\n"
            )
            image_context = image_context + "**Section:**  " + image["section"] + "\n"
            image_context = (
                image_context + "**Image Path:**  " + image["image_path"] + "\n"
            )
            image_context = (
                image_context + "**Image Caption:** " + image["caption"] + "\n\n"
            )
...
def context_retrieval(
    ...
    text_results, _ = similarity_search(
        query_embed,
        text_embeddings,
        text_content_list,
        k=text_k,
    ...
    image_results, _ = similarity_search(
        query_embed,
        image_embeddings,
        image_content_list,
        k=image_k,
